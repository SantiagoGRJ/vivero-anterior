        ...
        <table class="min-w-full bg-white border border-gray-200">
            <thead>
                <tr class="bg-gray-200 text-left text-gray-600 uppercase text-sm leading-normal">
                    ...
                    <th class="py-3 px-6">Ultima Aplicación</th>
                    <th class="py-3 px-6">Proxima Aplicación</th>
            ...
            <tbody class="text-gray-600 text-sm font-light">
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="py-3 px-6">{{ $item->fecha_ultima_aplicacion }}</td>
                    <td class="py-3 px-6">{{ $item->proxima_aplicacion }}</td>




                    <td class="py-3 px-6">
                        <a href="{{ route('fertilizacion.show', $item->id) }}" class="p-2 bg-blue-500 rounded-md text-white">Mostrar</a>
                    </td>
                    <td class="py-3 px-6">
                        <a href="{{ route('fertilizacion.edit', $item->id) }}" class="p-2 bg-green-500 rounded-md text-white">Editar</a>
                    </td>
                    <td class="py-3 px-6">
                        <form action="{{ route('fertilizacion.destroy', $item->id) }}" method="post">
                            @csrf
                            @method('DELETE')
